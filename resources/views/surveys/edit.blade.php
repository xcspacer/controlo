<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Editar Registo | {{ $survey->station->name }}
        </h2>
    </x-slot>
    <div class="py-12" x-data="surveyEditForm()" x-init="window.surveyFormInstance = $data">
        <div class="sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">

                    <form action="{{ route('surveys.update', $survey) }}" method="POST" @submit="prepareFormData()">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg">
                            <div>
                                ...
                                <div class="w-full p-2 bg-gray-100 rounded border">{{ $survey->station->name }}</div>
                                <input type="hidden" name="station_id" value="{{ $survey->station_id }}">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Mês</label>
                                <div class="w-full p-2 bg-gray-100 rounded border">
                                    {{ \Carbon\Carbon::create()->month($survey->month)->translatedFormat('F') }}
                                </div>
                            ...
                            <div>
                                ...
                                <div class="w-full p-2 bg-gray-100 rounded border">{{ $survey->year }}</div>
                                <input type="hidden" name="year" value="{{ $survey->year }}">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Dias no mês</label>
                                <div class="w-full p-2 bg-gray-100 rounded border">{{ $survey->days_in_month }}</div>
                                <input type="hidden" name="days_in_month" value="{{ $survey->days_in_month }}">
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('surveys.show', $survey) }}" class="mr-3 underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800">
                                Cancelar
                            </a>
    ...
    <script>
        ...
        function surveyEditForm() {
            return {
                form: {
                    station_id: '{{ $survey->station_id }}',
                    month: '{{ $survey->month }}',
                    year: {{ $survey->year }},
                    days_in_month: {{ $survey->days_in_month }},
                    readings: @json($survey->readings)
                },
                fuels: @json($survey->station->fuels),
                selectedStationName: '{{ $survey->station->name }}',

                init() {
                    // Ensure stock_entries structure exists for all fuels
                    this.fuels.forEach(fuel => {
                        if (!this.form.readings[fuel.id]) {
                        ...
                        if (!this.form.readings[fuel.id].stock_entries) {
                            this.form.readings[fuel.id].stock_entries = { values: {} };
                        }
                    });
                },
                ...
                generateTable() {
                    ...
                    html += '<tr>';
                    html += '<th class="border border-gray-300 px-3 py-2 text-sm font-medium text-left sticky left-0 bg-gray-100 z-30" style="min-width: 200px;" rowspan="2"></th>';
                    for (let day = 1; day <= this.form.days_in_month; day++) {
                        html += `<th class="border border-gray-300 px-1 py-1 text-xs text-center bg-gray-100" style="min-width: 120px;" colspan="2">${day}</th>`;
                    }
                    html += '</tr>';
                    html += '<tr>';
                    for (let day = 1; day <= this.form.days_in_month; day++) {
                        html += '<th class="border border-gray-300 px-1 py-1 text-xs bg-gray-100">Valores</th>';
                        html += '<th class="border border-gray-300 px-1 py-1 text-xs bg-gray-100">Total</th>';
                    }
                    html += '</tr>';
                    html += '</thead>';
                    ...
                    this.fuels.forEach(fuel => {
                        ...
                        for (let counter = 1; counter <= fuel.counter; counter++) {
                            ...
                            for (let day = 1; day <= this.form.days_in_month; day++) {
                                ...
                                const currentValue = this.form.readings[fuel.id]?.counters[counter-1]?.values[day] || '';
                                html += '<td class="border border-gray-300 p-0">';
                                html += `<input type="number" id="${inputId}" value="${currentValue}" `;
                                ...
                                html += `step="1" min="0" placeholder="0" onblur="window.updateCounterValue('${fuel.id}', ${counter-1}, ${day}, this.value)">`;
                                html += '</td>';
                                html += `<td class="border border-gray-300 p-1 bg-gray-50 text-center">`;
                                const total = this.form.readings[fuel.id]?.counters[counter-1]?.totals[day] || '0';
                                html += `<span class="text-xs" id="total_counter_${fuel.id}_${counter-1}_${day}">${total}</span>`;
                                html += '</td>';
                            }
                            html += '</tr>';
                        }
                        ...
                        for (let day = 1; day <= this.form.days_in_month; day++) {
                            ...
                            const currentValue = this.form.readings[fuel.id]?.sounding?.values[day] || '';
                            html += '<td class="border border-gray-300 p-0">';
                            html += `<input type="number" id="${inputId}" value="${currentValue}" `;
                            html += 'class="w-full text-xs p-1 text-center border-0 focus:ring-1 focus:ring-blue-500" ';
                            html += 'style="min-width: 58px; height: 30px; background: white;" ';
                            html += `step="1" min="0" placeholder="0" onblur="window.updateSoundingValue('${fuel.id}', ${day}, this.value)">`;
                            html += '</td>';
                            html += `<td class="border border-gray-300 p-1 bg-gray-300 text-center">`;
                            const total = this.form.readings[fuel.id]?.sounding?.totals[day] || '0';
                            html += `<span class="text-xs" id="total_sounding_${fuel.id}_${day}">${total}</span>`;
                            html += '</td>';
                        }
                        html += '</tr>';
                    ...
                    setTimeout(() => {
                        this.calculateAllInitialValues();
                    }, 100);

                    return html;
                },

                calculateAllInitialValues() {
                    this.fuels.forEach(fuel => {
                        for (let day = 1; day <= this.form.days_in_month; day++) {
                            for (let counter = 0; counter < fuel.counter; counter++) {
                                this.calculateCounterTotal(fuel.id, counter, day);
                            }
                            this.calculateSoundingTotal(fuel.id, day);
                        }
                        
                        for (let day = 1; day <= this.form.days_in_month; day++) {
                            this.calculateLitrosVendidos(fuel.id, day);
                            this.calculateResultadoAB(fuel.id, day);
                        }
                    });
                },
                ...
                initializeStockEntries() {
                    this.fuels.forEach(fuel => {
                        if (this.form.readings[fuel.id] && !this.form.readings[fuel.id].stock_entries) {
                            this.form.readings[fuel.id].stock_entries = {
                                values: {},
                                totals: {}
                            };
                        }
                ...
                recalculateAllDaysFromDay(fuelId, startDay) {
                    for (let day = startDay; day <= this.form.days_in_month; day++) {
                        this.calculateSoundingTotal(fuelId, day);
                    }
                },

                prepareFormData() {
                    this.fuels.forEach(fuel => {
                        // Initialize stock_entries structure if it doesn't exist
                        if (!this.form.readings[fuel.id].stock_entries) {
                            this.form.readings[fuel.id].stock_entries = { values: {} };
                        }
                    ...
                    try {
                        const jsonString = JSON.stringify(this.form.readings);
                        this.$refs.readingsInput.value = jsonString;
                        console.log('Form data prepared successfully');
                    } catch (error) {
                        console.error('Error preparing form data:', error);
                        alert('Erro ao preparar os dados do formulário. Por favor, tente novamente.');
                        return false;
                    }
                }
            }
