                ...
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    <form method="POST" action="{{ route('stations.update', $station) }}">
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="name" class="block mt-1 w-full dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300" type="text" name="name" :value="old('name', $station->name)" required autofocus />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="group" :value="__('Grupo')" class="dark:text-gray-300" />
                            <x-text-input id="group" class="block mt-1 w-full dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300" type="number" name="group" min="1" :value="old('group', $station->group)" required />
                            <x-input-error :messages="$errors->get('group')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="address" :value="__('Morada')" class="dark:text-gray-300" />
                            <x-text-input id="address" class="block mt-1 w-full dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300" type="text" name="address" :value="old('address', $station->address)" required />
                            <x-input-error :messages="$errors->get('address')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="city" :value="__('Localidade')" class="dark:text-gray-300" />
                            <x-text-input id="city" class="block mt-1 w-full dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300" type="text" name="city" :value="old('city', $station->city)" required />
                        ...
                        <div class="mt-4">
                            <label for="is_active" class="inline-flex items-center">
                                <input id="is_active" type="checkbox" class="rounded border-gray-300 dark:border-gray-600 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:bg-gray-700 dark:ring-offset-gray-800" name="is_active" {{ $station->is_active ? 'checked' : '' }}>
