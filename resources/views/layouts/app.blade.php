...
<body class="font-sans antialiased" x-init="$watch('darkMode', val => localStorage.setItem('darkMode', val)); if (!localStorage.getItem('darkMode') && window.matchMedia('(prefers-color-scheme: dark)').matches) { darkMode = true; }">
    ...
    <div x-data="{ show: true }"
        ...
        @click="show = false"
        class="fixed bottom-4 right-4 z-50 max-w-sm rounded-md shadow-lg p-4 {{ session('success') ? 'bg-green-50 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-900 text-red-800 dark:text-red-200' }}">
        ...
        <div class="flex items-center">
            ...
            <p class="ml-2 text-sm font-medium">{{ session('success') }}</p>
            @endif
            ...
            <p class="ml-2 text-sm font-medium">{{ session('error') }}</p>
            @endif
        </div>
    </div>
    @endif
